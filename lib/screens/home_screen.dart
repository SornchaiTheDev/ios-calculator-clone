...
class _MyCalculatorState extends State<MyCalculator> {
    ...
    try {
      ...
      setState(() {
        if (isIntResult) {
          ...
          int resultLastIndex = intResult.toString().length;
          int subStringEndIndex = resultLastIndex > 9 ? 9 : resultLastIndex;

          if (isLargeResult) {
            finalResult = intResult.toStringAsExponential(3);
          } else {
            finalResult = intResult.toString().substring(0, subStringEndIndex);
          }
        } else {
          int resultLastIndex = result.toString().length;
          int subStringEndIndex = resultLastIndex > 9 ? 9 : resultLastIndex;

          if (isLargeResult || isTinyResult) {
            finalResult = result.toStringAsExponential(3);
          } else {
            finalResult = result.toString().substring(0, subStringEndIndex);
          }
        }
  ...
  void handleOnPressOperator(String operator) {
    bool isMinus = operator == "-";

    if (expression.isEmpty && !isMinus) return;
